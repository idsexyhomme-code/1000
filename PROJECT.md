...
- **Git:** 매 이터레이션 끝에 의미 있는 단위로 `git add -A && git commit` → 원격(origin)이 있으면 `git push`. 사용자가 모바일에서 `git pull`로 진행 확인. 원격 없으면 로컬 커밋만(나중에 일괄 푸시). **API 키/시크릿 절대 커밋 금지**(.gitignore로 보호, 커밋 전 `git grep AIzaSy`로 스캔).
