...
PROBE_ATLAS: dict[str, dict] = {
    ...
    "copywriter": {
        "micro_itches_ko": [
            "AI로 뽑은 카피가 브랜드 톤·금지표현과 어긋나 통째로 다시 손본다.",
            "같은 상품을 채널·타깃·길이별로 수십 개 변형 카피로 찍어낸다.",
            "'느낌이 좀…' 같은 모호한 피드백을 구체적 수정 방향으로 번역해야 한다.",
            "A/B 테스트용 헤드라인을 계속 새로 짜내야 한다.",
            "법적 금지문구·과장광고 표현을 매번 직접 검수한다.",
            "기존 캠페인 톤과 일관되게 맞추느라 과거 자료를 다시 뒤진다.",
            "SEO 키워드를 자연스럽게 녹이면서 가독성도 지켜야 한다.",
            "경쟁사와 비슷해 보이지 않게 차별화 표현을 고민한다.",
            "납기에 쫓겨 단순 변형·규격 맞추기에 시간을 다 쓴다.",
            "클라이언트가 톤을 반려하면 어디가 문제인지 추측해 다시 쓴다.",
        ],
        "interview_questions_ko": [
            "AI 초안을 받은 뒤 가장 많이 손보는 부분은 무엇인가요?",
            "한 캠페인에서 카피 변형은 보통 몇 개나 만드나요?",
            "클라이언트 피드백 중 해석이 가장 어려운 표현은 무엇인가요?",
            "브랜드 톤·금지표현 검수에 얼마나 시간이 드나요?",
            "먼저 필요한 결과물은 톤 교정안, 채널별 변형표, 피드백 해석표 중 무엇인가요?",
        ],
        "money_moment_ko": "캠페인 런칭·대량 카피 납품처럼 톤 일관성·변형 물량·금지표현 검수가 폭증하는 순간",
        "wedge_ko": "브랜드 보이스·금지표현을 결박해 카피 초안을 톤에 맞게 교정하고, 채널·길이별 변형안으로 펼친다.",
        "avoid_ko": "최종 카피를 자동 발행하거나 광고 적법성을 보증하지 않고, 톤 교정안과 위반 표시만 제공한다.",
    },
    "office-admin": {
        "micro_itches_ko": [
