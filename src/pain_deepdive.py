...
DEEP_ATLAS: dict[tuple[str, str], dict] = {
    ...
    ("bookkeeper", "reconciliation-mismatch"): {
        "burning_moment_ko": "마감 직전 통장과 장부 잔액이 안 맞는데, 어느 전표가 원인인지 안 보여 한 건씩 거슬러 올라가는 순간",
        "hidden_cost_ko": "대사 차이 추적은 청구하기 어려운 시간이지만, 결산 지연과 잘못된 재무수치로 바로 이어진다.",
        "bad_ai_trap_ko": "일반 AI는 계정·거래처·날짜·금액을 교차로 묶어 의심 전표를 좁히지 못하고, 회계 규칙과 증빙 맥락을 놓친다.",
        "minimum_inputs_ko": ["통장/카드 거래내역", "장부 전표 내역", "대사 기준 계정", "차이 발생 기간"],
        "first_relief_ko": "10분 안에 '금액·계정·거래처가 어긋난 의심 전표 후보'가 확인 순서와 함께 한 장으로 나온다.",
        "paid_trigger_ko": "월마감·분기결산처럼 잔액이 반드시 맞아야 하고 차이 추적 시간이 폭증할 때",
        "pilot_script_ko": "통장과 장부 차이를 의심 전표 후보로 좁혀주는 월말 대사 정리 파일럿",
        "success_metric_ko": "대사 차이 원인 탐색 시간 절감, 의심 전표 후보 정확도, 결산 마감 지연 감소",
        "human_boundary_ko": "전표를 자동 수정·확정하지 않고, 운영자가 검토할 의심 후보와 확인 순서만 정리한다.",
    },
    ("copywriter", "brand-voice-drift"): {
        "burning_moment_ko": "AI로 빨리 뽑은 카피가 그럴듯한데, 브랜드 보이스·금지표현·기존 캠페인 톤과 어긋나 통째로 다시 쓰는 순간",
        "hidden_cost_ko": "글 쓰는 시간보다 '톤을 맞추고 위반을 걸러내는' 교정 시간이 성과를 잡아먹고, 톤이 흔들리면 브랜드 신뢰가 깎인다.",
        "bad_ai_trap_ko": "일반 프롬프트는 브랜드 보이스 가이드, 법적 금지문구, 과거 캠페인 톤, 경쟁사 회피 표현을 동시에 지키지 못한다.",
        "minimum_inputs_ko": ["브랜드 보이스 가이드/톤 예시", "금지·주의 표현 목록", "레퍼런스 카피 2~3개", "이번 카피의 채널·타깃"],
        "first_relief_ko": "10분 안에 '브랜드 톤에 맞게 1차 교정된 카피 + 위반·어긋난 표현 표시'가 한 장으로 나온다.",
        "paid_trigger_ko": "캠페인 런칭·대량 카피 납품처럼 톤 일관성과 금지표현 검수 비용이 폭증할 때",
        "pilot_script_ko": "브랜드 보이스·금지표현을 결박해 카피 초안을 톤에 맞게 교정하는 브랜드 톤 정리 파일럿",
        "success_metric_ko": "톤 재작업 횟수 감소, 금지표현 누락 0, 클라이언트 톤 반려 감소",
        "human_boundary_ko": "최종 카피를 자동 발행하지 않고, 운영자가 검토할 톤 교정안과 위반 표시만 제공한다.",
    },
